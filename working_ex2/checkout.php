...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (get_magic_quotes_gpc()) {
		$_POST['first_name'] = stripcslashes($_POST['first_name']);
		// repeat for other variables tha could be affected
	}
	...
	if (preg_match('/^[A-Z \'.-]{2,60}$/i', $_POST['state'])) {
		...
		if (empty($shipping_errors)) {
			...
			trigger_error('Your order could not be processed due to a system error. We apologize for the incnvenience.');
		} // errors occurred if
