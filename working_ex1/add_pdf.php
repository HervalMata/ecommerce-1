...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (empty($add_pdf_errors)) {
		...
		if (mysqli_affected_rows($dbc) === 1) {
			...
			$_POST = array();
			$_FILES = array();
			enset($file, $_SESSION['pdf']);
		} else { // if it did not run OK
			trigger_error('The PDF cold not be added due to a system error. We apologize for any inconvenience.');
			unlink ($dest);
		}
	} else {
		unset($_SESSION['pdf']);
	...
	<form enctype="multipart/form-data" action="add_pdf.php" method="post" accept-charset="utf-8">
		...
		if (array_key_exists('pdf', $add_pdf_errors)) {
			...
			if (isset($_SESSION['pdf'])) {
				echo '<p class="lead">Currently: "' . $_SESSION['pdf']['filename'] . '"</p>';
			}
		}
	} // end of errors if-else
	echo '<span class="help-block">PDF only, SMB Limit</span></div>';
	?>
