...
if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['txn_id]']) && ($_POST['txn_type'] === 'web_accept')) {
	...
	array (
		...
		CURLOPT_POST => true,
		CURLOPT_POSTFIELDS => http_build-query(array('cmd' => '_notify-validate') + $_POST),
	...
	if ($status === 200 && $response === 'VERIFIED') {
		if (isset($_POST['payment_status'])
			...
			&& ($_POST['receiver_email'] === 'sb-bxc9k553696@business.example.com')
			...
			if (mysqli_num_rows($r) === 0) {
				...
				if (mysqli_affected_rows($dbc) === 1) {
					if ($uid > 0) {
						$q = "UPDATE users SET date_expires = IF(date_expires > NOW(), ADDATE(date_expires, INTERVAL 1 YEAR), ADDATE(NOW(), INTERVAL 1 YEAR)), date_modified=NOW() WHERE id=$uid";
						$r = mysqli_query($dbc, $q);
						if (mysqli_affected_rows($dbc) !== 1) {
